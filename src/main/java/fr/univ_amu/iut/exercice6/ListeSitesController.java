...
public class ListeSitesController {
  ...
  public int ajouterSiteDemo() {
    ...
    // ajouts.
    SiteCarte mySiteCarte = new SiteCarte();
    mySiteCarte.setNumeroCarre("Carré " + (640000 + compteurDemo));
    mySiteCarte.setNomConvivial("📍 Site de démonstration #" + compteurDemo);
    mySiteCarte.setNombrePoints((compteurDemo % 3) + 1);
    mySiteCarte.setNombrePassages(compteurDemo * 2);
    mySiteCarte.setJoursDepuisDernierPassage(compteurDemo * 4);
    // 3. Ajouter la carte au début (index 0) du VBox pour que les nouveaux sites
    // apparaissent en
    // haut, comme dans un flux d'activité.
    conteneurCartes.getChildren().add(mySiteCarte);
